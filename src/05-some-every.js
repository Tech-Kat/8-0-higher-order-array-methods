...
function allSongsAreOverTwoMinutes(songs) {
  return songs.every((song) => {return song.runtimeInSeconds > 120})}
...
function anySongIsOverFourMinutes(songs) {
  return songs.some((song) => {return song.runtimeInSeconds > 240})
}
...
function anySongIsByPeanut(songs) {
  return songs.some((song) => {return song.artist === "Peanut"})
}
