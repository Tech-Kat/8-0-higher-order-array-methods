 ...
 */
function getSongTitles(songs) {let titleOfSongs = songs.map((song) => {return song.title}) 
return titleOfSongs;
}
 ...
 */
function getSongDetails(songs) {let artistTitle = songs.map((song) => {return `${song.title} by ${song.artist}`})
return artistTitle;
}
 ...
 */
function getTitleAndArtist(songs) {let titleKeys = songs.map((song) => {return {[song.title]: song.artist};})
return titleKeys
}
