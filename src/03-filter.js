...
function getSongsBySaib(songs) {
  return songs.filter((song) => { return song.artist === "Saib";
    })
}
...
function getSongsOverThreeMinutes(songs) {
  return songs.filter((song) => { return song.runtimeInSeconds > 180})
}
...
function getTitleTracks(songs) {
  return songs.filter((song) => {return song.title === song.album})
}
